  ...
  const title = (
    <FormattedMessage
      ...
      values={{
        ...
        thing: <BusinessListSelector selected="no-plastic-bottles" />,
      }}
    />
  );
  return (
      ...
      <Helmet>
        <title>
          {formatMessage(
            ...
            {
              city: formatMessage({ id: 'Saigon' }),
              thing: formatMessage({ id: 'no_plastic_bottles_label' }).toLowerCase(),
            },
          )}
        </title>
      </Helmet>
...
export const query = graphql`
  {
    allAirtable(
      filter: {
        ...
        data: { Status: { eq: "Published" }, No_plastic_bottles: { eq: true } }
      }
